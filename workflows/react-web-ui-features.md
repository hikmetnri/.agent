...
- 2026-07-08: Bug düzeltme turu tamamlandı ve GitHub'a push edildi. `UserExamSolve`: guestToast anti-pattern, mode logic (testType öncelikli), persistedTestType fallback. `UserExams`: geçersiz Tailwind class, shortGroups destructuring, filtre testType güçlendirmesi, kart soru sayısı etiketi. `UserLessons`: geçersiz Tailwind class (4 yer), çift progress bar, ders sayısı badge düzeltmesi, voice menu dış tıklama kapanma.
- 2026-07-04: Büyük UI yenileme turu tamamlandı. React web: `UserHome` masaüstü dashboard tam yeniden tasarım (gradient banner, misafir CTA, 3 sütun grid, "Son Sınavlarım" widget, "Günün Sözü" premium kart); `UserExams` hero banner + sliding-pill sekmeler; `UserFeed` pagination (PAGE_SIZE=15); `UserLessons` sidebar progress bar + premium header; `UserSettings` heatmap gerçek veri + rozet icon render düzeltmesi + liderboard isSelf fix. Admin web: `AdminBadges`, `AdminFeed`, `AdminReports`, `AdminSupport`, `AdminProfile` tam yeniden tasarım; `AdminMarketing` ve `AdminNotifications` bağımsız sayfalara ayrıldı. Güvenlik: `api/index.js` 401 interceptor Zustand store sıfırlar, `authStore` logout tüm storage'ı temizler, `AdminUsers` kendi rolünü değiştirme koruması. Backend (c5fc187): helmet + rate-limit + compression + CORS + payload sınırı; `badgeAwardService` rozet kriterleri; `notificationController` broadcast geçmişi + FCM push; `postController` validasyon + yorum bildirimleri; `examResultController` skor normalizasyonu + liderboard; `authController` profil/şifre/avatar/silme güçlendirildi.
